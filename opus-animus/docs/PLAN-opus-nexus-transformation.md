...
- **Opus** = whole second brain / personal operating system.
- **Nexus** = main app shell and conversational command center.
...
Do not delete Vita. Demote it from whole app name to module inside Nexus.
...
```text
🌐 Opus Nexus
Tabs: Today | Vita | Calendar | Actions
```
...
Displays raw personal data: calories, macros, sleep, steps, weight, workout, finance.

### Level 2 — Dashboard

Adds today summary, trends, progress, empty states, insight cards.

### Level 3 — Assistant

Adds chat / command input, daily brief, weekly review, calendar-aware advice.

### Level 4 — Operator

Adds LLM-proposed actions, user approval, and writes approved events to Google Calendar.

### Target

Move from Level 1/2 to Level 3.5:
...
Today screen should answer within 5 seconds:
...
Vita is health/life data module preserved from `opus-vita`.
...
Nexus creates Google Calendar events from LLM proposals after user approval.
...
2. Nexus reads:
   - calendar for next 7–14 days
   - health logs
   - workout logs
   - finance / eating signals
   - user goals and preferences
3. Nexus creates weekly plan:
   - workout events
...
Generated when the user opens the app, not necessarily as push notification in MVP.
...
At week end, Nexus summarizes life operation signals.
...
Weekly review should be convertible into calendar proposal.
...
Before showing or approving proposal, validate:
...
Keep current dark bento style, but rename visual language:
...
First real MVP should build only:
...
Use this prompt to create the code implementation plan.
