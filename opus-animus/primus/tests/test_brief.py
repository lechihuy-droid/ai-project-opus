...
def test_generate_brief_includes_actio_finance_signal(monkeypatch, tmp_path):
    _set_brief_env(monkeypatch, tmp_path)
    monkeypatch.setattr(rector.tasks, "pull_due_tasks", lambda date, profile: [])
    monkeypatch.setattr(
        providers,
    ...
    monkeypatch.setattr(
        providers,
        "actio_finance",
        lambda active_goals, date=None: [
            {
                "title": "Cash drag",
                "reason": "76% net worth idle vs house-fund target.",
                "priority": "high",
                "suggested_action": "Deploy idle cash into core allocation.",
    ...
    packet = _intent_packet()
    packet["profile"] = {"active_goals": ["finance_freedom"]}
    _, items = generate_brief(packet)

    finance = [item for item in items if item["source_subsystem"] == "ACTIO"]
    assert len(finance) == 1
    assert finance[0]["kind"] == "finance_alert"
    assert finance[0]["requires_approval"] is True
