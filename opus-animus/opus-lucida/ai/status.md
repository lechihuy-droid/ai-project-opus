...
- **M6 HOÀN TẤT build (2026-07-16):** RD `docs/RD-visual-mechanism.md` → BD `docs/BD-visual-mechanism.md`, tất cả phase A/B/C1/C2/C2.1/C2.2/C2.3 ✅ (kể cả 1 bug schema thật phát hiện khi render map thật — `remove` transition bị bắt buộc `props` sai, đã vá). Gate 2 (video-map) user đã duyệt 2026-07-15 sau khi Sonnet phát hiện + fix 3 lỗi layout (chip chồng, timer đè subtitle, follow chip đè header).
- **Video final email-keigo v2 render xong 2026-07-16:** `output/render/flow-runs/email-keigo-v2/video.mp4` (58s, 8.1MB) + publish bundle `output/publish/email-keigo-v2/`. Report: audio OK, voice checksum khớp v1, drift 13.33ms, **semantic QA 4 pass/0 warn/0 fail** (v1 từng là 1 FAIL "ba bước"/4 steps + 4 WARN). Claude tự QA 6 still trên map đã apply-timing thật — sạch.
- **Gate 3 lần 2 (2026-07-16): user CHƯA duyệt** — feedback: chủ đề mail+AI nhưng chỉ có 1 cửa sổ đổi vai (MAIL → "AI PROMPT" bằng đổi title), thiếu **cửa sổ mail và terminal AI riêng biệt**. Phân loại: lỗi (b) component gap — continuous mode chỉ hỗ trợ 1 environment window, không add được window thứ hai như element. → Cần M6.1: MechanismWindow addable làm element (AI terminal variant chat/terminal) + beat "mail thu gọn ↔ terminal xuất hiện ↔ kết quả áp ngược vào mail".
- **FLOW_V1.md đã chính thức hóa 2 vòng lặp sản xuất (mục 2b, 2026-07-16):** Loop 1 map-level (validators + still storyboard QA, rẻ, lặp nhiều) → gate 2; Loop 2 video-level (flow:run + QA trên map apply-timing thật) → gate 3; kèm quy tắc phân loại lỗi (a) map / (b) component / (c) script khi user chê.
- **Hiện trạng automation (2026-07-16):** từng bước đã CLI-hóa đủ (`voice:generate`, `voice:align`, `validate:brand`, `validate:semantic`, `flow:run` = apply-timing→validate→render→publish, `flow:server` :8790 với `/health` + `/run`); n8n có `n8n/docker-compose.yml` + workflow importable `n8n/workflows/lucida-flow.json`; **Docker ĐÃ cài (29.6.1)** — risk cũ "Docker not installed" hết hiệu lực, n8n giờ khởi động được nhưng CHƯA chạy thật lần nào; still-storyboard loop chưa nằm trong flow:run (đang là bước Claude/Sonnet thủ công).
- Email JP dùng trong video: `input/scripts/ai-email-keigo/email-jp-sample.md` (draft gốc, đã đưa vào diff-highlight).
