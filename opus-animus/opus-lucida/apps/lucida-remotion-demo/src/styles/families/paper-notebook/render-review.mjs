...
const packageDir = path.resolve(root, "design/visual-library/styles/paper-notebook");
...
const projectRelative = (absolutePath) => path.relative(root, absolutePath).split(path.sep).join("/");
...
const main = async () => {
  ...
  const serveUrl = await bundle({ entryPoint: path.resolve(root, renderProps.entryPoint), webpackOverride: (config) => config });
  const frames = [];

  for (const scene of demo.scenes) {
    const inputProps = { sceneKey: scene.sceneKey };
    const composition = await selectComposition({ serveUrl, id: renderProps.compositionId, inputProps, browserExecutable, logLevel: "error" });
    const outputPath = path.resolve(artifactsDir, scene.output);
    await renderStill({ serveUrl, composition, inputProps, output: outputPath, frame: scene.frame, imageFormat: renderProps.imageFormat, overwrite: true, browserExecutable, logLevel: "error" });
    const stats = signalStats(outputPath);
    const dynamicRange = Number((stats.yHigh - stats.yLow).toFixed(2));
    frames.push({
      id: scene.id, label: scene.label, sceneKey: scene.sceneKey, frame: scene.frame,
      output: projectRelative(outputPath),
      dimensions: { width: composition.width, height: composition.height },
      durationInFrames: composition.durationInFrames,
      sha256: hashFile(outputPath), fileBytes: fs.statSync(outputPath).size,
      signalStats: stats,
      nonBlank: { passed: dynamicRange >= 8 || stats.saturationAverage >= 3, heuristic: "signalstats dynamic range or saturation threshold", dynamicRange },
    });
  }

  const report = {
    status: frames.length === 3 && frames.every((item) => item.nonBlank.passed) ? "ok" : "failed",
    packageId: demo.packageId,
    compositionId: renderProps.compositionId,
    generatedAt: new Date().toISOString(),
    renderer: { entryPoint: renderProps.entryPoint, browserExecutable, imageFormat: renderProps.imageFormat, rasterDependencies: false },
    ...
    summary: {
      ...
      dimensionsValid: frames.every((item) => item.dimensions.width === demo.width && item.dimensions.height === demo.height),
    },
  ...
  process.stdout.write(`Rendered ${frames.length} paper-notebook review frames -> ${projectRelative(framesDir)}\n`);
  if (report.status !== "ok" || !report.summary.dimensionsValid) process.exit(1);
};
