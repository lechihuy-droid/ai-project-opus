  ...
  for (const sceneItem of root.scenes) {
    for (const layer of sceneItem.layers || []) {
      if (layer.enter) {
        ...
        css.push(`#${cssIdentifier(layer.id)} { opacity: 0; animation: ${name} ${formatNumber(enter.duration)}s ${enter.ease} ${formatNumber(delay)}s both; }`);
      ...
      if (layer.emphasis) {
        ...
        css.push(`#${cssIdentifier(layer.id)} { animation: ${name} ${formatNumber(emphasis.duration)}s ${emphasis.ease} ${formatNumber(delay)}s ${emphasis.iterations || 1} both; }`);
