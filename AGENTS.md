...
**Slash command:** `/html [mô tả]` — available in Codex
...
> - Tiếp tục → tôi sẽ đọc `ai/status.md` và `ai/handoff-AGENTS.md` của project hiện tại
...
2. Read `ai/handoff-AGENTS.md` (owner Codex) HOẶC `ai/handoff-codex.md` (owner Codex theo status.md) — exact next action
...
Slash command `/handoff` (global, ở `~/.Codex/commands/handoff.md`) sẽ:
1. Auto-detect project root (folder gần nhất có `ai/`)
2. Overwrite `ai/handoff-AGENTS.md` (Codex) theo schema
