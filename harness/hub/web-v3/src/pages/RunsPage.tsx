...
export default function RunsPage() {
  ...
  return <div className="flex h-full min-h-0 flex-col gap-3 p-3 md:p-4">
    <form onSubmit={e => { e.preventDefault(); void launch() }} className="flex flex-wrap items-end gap-2 rounded-[var(--hub-radius-lg)] border border-line bg-panel p-3"><label className="flex min-w-[180px] flex-1 flex-col gap-1 text-[length:var(--hub-section-size)] font-semibold uppercase tracking-[var(--hub-section-tracking)] text-faint">Workflow<Select value={workflowId} onChange={e => setWorkflowId(e.target.value)}><option value="">Chọn workflow</option>{workflows.map(w => <option key={w.id} value={w.id}>{label(w)}</option>)}</Select></label><label className="flex min-w-[240px] flex-[2] flex-col gap-1 text-[length:var(--hub-section-size)] font-semibold uppercase tracking-[var(--hub-section-tracking)] text-faint">Mục tiêu<Textarea value={objective} onChange={e => setObjective(e.target.value)} rows={1} placeholder="Mục tiêu cho phiên chạy…" /></label><Button variant="primary" type="submit" disabled={busy || !workflowId || !objective.trim()}>{busy ? 'Đang chạy…' : 'Chạy'}</Button></form>
...
function RecentRuns({ rows, onChoose, disabled }: { rows: RunState[]; onChoose: (row: RunState) => void; disabled: boolean }) { const compact = useMemo(() => rows.slice(0, 5), [rows]); return <section className="rounded-[var(--hub-radius-lg)] border border-line bg-panel p-3"><div className="mb-2 text-[length:var(--hub-section-size)] font-semibold uppercase tracking-[var(--hub-section-tracking)] text-faint">Phiên chạy gần đây</div><div className="flex flex-wrap gap-2">{compact.map(row => <Button variant="secondary" size="sm" disabled={disabled} key={row.run_id} onClick={() => onChoose(row)} className="min-w-[180px] text-left"><span><span className="block truncate text-text">{String(row.metadata?.objective ?? row.run_id)}</span><span className="font-mono text-[10px] text-dim">{row.status} · {String(row.metadata?.updated_at ?? row.metadata?.created_at ?? '')}</span></span></Button>)}</div></section> }
