...
export default function RunsPage() {
  ...
  return <div className="flex h-full min-h-0 flex-col gap-3 p-3 md:p-4">
    <form onSubmit={e => { e.preventDefault(); void launch() }} className="flex flex-wrap items-end gap-2 rounded-lg border border-line bg-panel p-3"><label className="flex min-w-[180px] flex-1 flex-col gap-1 text-[10px] font-semibold uppercase tracking-wide text-faint">Workflow<select value={workflowId} onChange={e => setWorkflowId(e.target.value)} className="rounded-lg border border-line bg-panel2 px-2 py-2 text-xs font-normal normal-case tracking-normal text-text"><option value="">Chọn workflow</option>{workflows.map(w => <option key={w.id} value={w.id}>{label(w)}</option>)}</select></label><label className="flex min-w-[240px] flex-[2] flex-col gap-1 text-[10px] font-semibold uppercase tracking-wide text-faint">Mục tiêu<textarea value={objective} onChange={e => setObjective(e.target.value)} rows={1} placeholder="Mục tiêu cho phiên chạy…" className="resize-y rounded-lg border border-line bg-panel2 px-2 py-2 text-xs font-normal normal-case tracking-normal text-text" /></label><button disabled={busy || !workflowId || !objective.trim()} className="rounded-lg bg-codex px-5 py-2 text-xs font-semibold text-ink disabled:opacity-40">{busy ? 'Đang chạy…' : 'Chạy'}</button></form>
    {error && <div className="flex items-center gap-3 text-xs text-err">{error}<button onClick={() => setError('')} className="text-dim">Ẩn</button></div>}
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-line lg:flex-row"><div className="flex min-h-0 min-w-0 flex-1 flex-col"><div className="flex items-center justify-between border-b border-line bg-panel px-4 py-2"><span className="text-xs font-semibold">Spine</span><span className="font-mono text-[10px] text-faint">{run ? `${run.run_id} · ${run.status}` : 'chưa có run'}</span></div><RunSpine nodes={nodes} run={run} interrupt={interrupt} onGate={approved => void gate(approved)} resuming={resuming} /></div><ArtifactRail run={run} events={events} live={busy} refreshKey={refreshKey} /></div>
    {recent.length > 0 && <RecentRuns rows={recent} onChoose={row => void reopen(row)} disabled={busy} />}
  </div>
}

function RecentRuns({ rows, onChoose, disabled }: { rows: RunState[]; onChoose: (row: RunState) => void; disabled: boolean }) { const compact = useMemo(() => rows.slice(0, 5), [rows]); return <section className="rounded-lg border border-line bg-panel p-3"><div className="mb-2 text-[10px] font-semibold uppercase tracking-[.14em] text-faint">Phiên chạy gần đây</div><div className="flex flex-wrap gap-2">{compact.map(row => <button disabled={disabled} key={row.run_id} onClick={() => onChoose(row)} className="min-w-[180px] rounded border border-line bg-panel2 px-2 py-1.5 text-left text-xs hover:border-codex disabled:opacity-50"><div className="truncate text-text">{String(row.metadata?.objective ?? row.run_id)}</div><div className="font-mono text-[10px] text-dim">{row.status} · {String(row.metadata?.updated_at ?? row.metadata?.created_at ?? '')}</div></button>)}</div></section> }
