...
import { Alert, Button, ListItem, Panel, RunStatusBadge, Select, Textarea, Toolbar } from '../lib/ui'
...
export default function RunsPage() {
  ...
  return <div className="flex h-full min-h-0 flex-col gap-space-3 p-space-3 md:p-space-4">
    <Panel bodyClassName="p-space-3"><form onSubmit={e => { e.preventDefault(); void launch() }}><Toolbar className="h-auto min-h-toolbar flex-wrap items-end gap-space-2"><label className="flex min-w-[180px] flex-1 flex-col gap-space-1 text-section font-semibold uppercase tracking-section text-muted">{t('runs.workflow')}<Select value={workflowId} onChange={e => setWorkflowId(e.target.value)}><option value="">{t('runs.selectWorkflow')}</option>{workflows.map(w => <option key={w.id} value={w.id}>{label(w)}</option>)}</Select></label><label className="flex min-w-[240px] flex-[2] flex-col gap-space-1 text-section font-semibold uppercase tracking-section text-muted">{t('runs.objective')}<Textarea value={objective} onChange={e => setObjective(e.target.value)} rows={1} placeholder={t('runs.objectivePlaceholder')} /></label><RunInputPicker value={inputs} onChange={setInputs} copy={{ add: t('runs.addInputs'), artifacts: t('runs.artifacts'), files: t('runs.files'), search: t('runs.searchInputs'), noArtifacts: t('runs.noArtifacts'), noFiles: t('runs.noFiles'), artifact: t('runs.inputArtifact'), file: t('runs.inputFile'), remove: t('runs.removeInput') }} /><Button variant="primary" type="submit" disabled={busy || !workflowId || !objective.trim()}>{busy ? t('runs.running') : t('runs.run')}</Button></Toolbar></form></Panel>
    {error && <Alert variant="error" onDismiss={() => setError('')}>{error}</Alert>}
    <Panel className="min-h-0 flex-1 overflow-hidden" bodyClassName="flex h-full min-h-0 flex-col p-0 lg:flex-row"><div className="flex min-h-0 min-w-0 flex-1 flex-col"><div className="flex items-center justify-between border-b border-border-subtle px-space-4 py-space-2"><span className="text-label font-semibold">{t('runs.spine')}</span>{run ? <RunStatusBadge kind={run.status === 'failed' ? 'error' : run.status === 'interrupted' ? 'interrupted' : run.status === 'running' ? 'running' : 'neutral'} label={`${run.run_id} · ${run.status}`} /> : <span className="font-mono text-caption text-muted">{t('runs.noRun')}</span>}</div><RunSpine nodes={nodes} run={run} interrupt={interrupt} onGate={approved => void gate(approved)} resuming={resuming} /></div><ArtifactRail run={run} events={events} live={busy} refreshKey={refreshKey} /></Panel>
    {recent.length > 0 && <RecentRuns rows={recent} onChoose={row => void reopen(row)} disabled={busy} />}
  </div>
}

function RecentRuns({ rows, onChoose, disabled }: { rows: RunState[]; onChoose: (row: RunState) => void; disabled: boolean }) { const compact = useMemo(() => rows.slice(0, 5), [rows]); return <Panel header={<div className="text-section font-semibold uppercase tracking-section text-muted">{t('runs.recent')}</div>} bodyClassName="flex flex-wrap gap-space-2">{compact.map(row => <ListItem disabled={disabled} key={row.run_id} onClick={() => onChoose(row)} className="min-w-[180px] flex-1" title={String(row.metadata?.objective ?? row.run_id)} description={`${row.status} · ${String(row.metadata?.updated_at ?? row.metadata?.created_at ?? '')}`} />)}</Panel> }
