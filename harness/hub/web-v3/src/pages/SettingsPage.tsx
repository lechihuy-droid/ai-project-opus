...
type FolderGrant = { workflow_id: string; path: string; granted_at: string; exists?: boolean }
...
export default function SettingsPage() {
  ...
  useEffect(() => { void Promise.all([api<Provider[]>('/api/providers'), api<Models>('/api/chat/models'), api<Health>('/api/health'), api<Retention>('/api/settings/retention'), api<FolderGrant[]>('/api/folder-grants')]).then(([p, m, h, r, g]) => { setProviders(p); setModels(m); setHealth(h); setRetention(r); setDays(r.days === undefined ? '' : String(r.days)); setGrants(g) }).catch(e => setError(e instanceof ApiError ? e.message : t('settings.loadFailed'))) }, [])
  ...
  const revoke = (grant: FolderGrant) => { void api(`/api/workflows/${encodeURIComponent(grant.workflow_id)}/folder-grants?path=${encodeURIComponent(grant.path)}`, { method: 'DELETE' }).then(() => setGrants(rows => rows.filter(row => row !== grant))).catch(e => setError(e instanceof ApiError ? e.message : t('settings.revokeGrantFailed'))) }
  ...
  return <div className="flex h-full min-h-0 flex-col gap-space-4 overflow-auto p-space-6">
    ...
    <Panel header={<><h2 className="text-label font-semibold">{t('settings.folderGrants')}</h2><p className="mt-space-1 text-caption text-secondary">{t('settings.folderGrantsDescription')}</p></>} bodyClassName="p-space-4"><div className="space-y-space-2">{grants.map(grant => <div key={`${grant.workflow_id}:${grant.path}`} className="flex flex-wrap items-center gap-space-2 rounded-md border border-border-subtle p-space-2 text-caption"><span className="font-mono text-primary">{grant.workflow_id}</span><span className="min-w-0 flex-1 break-all text-secondary">{grant.path}</span>{grant.exists === false ? <Chip>{t('settings.grantMissing')}</Chip> : null}<span className="text-muted">{grant.granted_at}</span><Button variant="ghost" size="sm" onClick={() => revoke(grant)}>{t('settings.grantRevoke')}</Button></div>)}</div></Panel>
  </div>
}
