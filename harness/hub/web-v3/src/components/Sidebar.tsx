...
export default function Sidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return <aside className="flex w-[240px] flex-col gap-0 overflow-y-auto border-r border-border-subtle bg-sidebar px-[10px] py-[14px]">
    <div className="flex items-center gap-[10px] px-[10px] pb-4 pt-2"><div className="grid h-[34px] w-[34px] shrink-0 place-items-center rounded-[var(--hub-radius-lg)] bg-[var(--hub-accent)] font-bold text-app">H</div><div className="sidebar-label"><div className="font-semibold">Harness Hub</div><Status kind="ready" label={providers ? `${providers.filter(provider => provider.available).length} provider online` : '—'} /></div><button type="button" className="sidebar-collapse ml-auto" aria-label={collapsed ? 'Mở rộng thanh điều hướng' : 'Thu gọn thanh điều hướng'} title={collapsed ? 'Mở rộng thanh điều hướng' : 'Thu gọn thanh điều hướng'} onClick={onToggle}>{collapsed ? '›' : '‹'}</button></div>
    {zones.map(zone => <div key={zone.label}><div className="sidebar-zone px-[10px] pb-[5px] pt-[14px] text-section font-semibold uppercase tracking-section text-muted">{zone.label}</div>{zone.items.map(([icon, label, to]) => <NavLink key={to} to={to} className={link} title={label} aria-label={label}><span className="w-4 text-center opacity-85">{icon}</span><span className="sidebar-label">{label}</span></NavLink>)}</div>)}
    {recent.length > 0 && <div className="sidebar-label"><div className="px-[10px] pb-[5px] pt-[14px] text-section font-semibold uppercase tracking-section text-muted">RECENT</div>{recent.map(item => <NavLink key={item.id} to="/artifacts" className={`${link} block truncate`} title={item.title}>{item.title}</NavLink>)}</div>}
