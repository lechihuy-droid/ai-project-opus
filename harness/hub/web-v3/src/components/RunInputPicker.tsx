...
import type { RunInput } from '../lib/runsApi'
...
const keyFor = (item: RunInput) => item.kind === 'artifact' ? `artifact:${item.id}` : item.kind === 'chat_file' ? `chat_file:${item.chat_id}:${item.name}` : `folder:${item.path}`
...
export default function RunInputPicker({ value, onChange, copy }: { value: RunInput[]; onChange: (value: RunInput[]) => void; copy: Copy }) {
  ...
  const name = (item: RunInput) => item.kind === 'artifact' ? artifacts.find(row => row.id === item.id)?.title ?? item.id : item.kind === 'chat_file' ? item.name : item.path
