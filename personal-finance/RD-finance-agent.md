...
**Hiện trạng:** Đã có hệ Vita Module (health) chạy ổn theo pattern *chat-first → JSON-on-GitHub → static dashboard*. Folder `finance-data/` đã tồn tại với 1 file thử nghiệm đúng pattern này. Policy bảo mật 2 lớp đã có (`FINANCE_DATA_STORAGE_POLICY.md`).
...
Bước 2: Agent extract → ghi finance-data/YYYY-MM-DD.json (GitHub MCP)
...
|---|---|---|---|
| FR-001 | Log expense/income từ ảnh (receipt, screenshot app thanh toán) hoặc text → `finance-data/YYYY-MM-DD.json` + cập nhật `index.json` | P0 | Pattern y hệt FR1 của health |
...
| FR-005 | Review tháng theo quy trình CFP: đọc data → diagnosis → cập nhật `personal-finance/snapshots/` (ranges only, theo policy) | P1 | Goal-based buckets kiểu UBS |
...
## 4. Data Schema (extend schema hiện có trong `finance-data/`)
...
- Số dư/lương KHÔNG nằm trong file ngày — chỉ nằm trong `personal-finance/snapshots/` dạng range.
